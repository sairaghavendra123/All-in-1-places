...
export const BAJJI_MIXTURE_SECTIONS = [
  {
    id: 'mixtures',
    icon: '🥗',
    title: 'Mixtures',
    subtitle: 'tossed in tangy muntha masala',
    items: [
      { name: 'Bajji Mixture', price: 49 },
      { name: 'Aratikaya Mixture', price: 49 },
      { name: 'Tomato Mixture', price: 49 },
      { name: 'Egg Mixture', price: 59 },
      { name: 'Marmarala Mixture', price: 49 },
      { name: 'Chips Mixture', price: 49 },
    ]
  },
  {
    id: 'bajji',
    icon: '🌶️',
    title: 'Bajji',
    subtitle: 'battered & deep-fried',
    items: [
      { name: 'Aloo Bajji', price: 49 },
      { name: 'Aratikaya Bajji', price: 49 },
      { name: 'Tomato Bajji', price: 49 },
      { name: 'Egg Bajji', price: 59 },
    ]
  },
  {
    id: 'masala',
    icon: '🔥',
    title: 'Masala',
    subtitle: 'spiced & crushed to order',
    items: [
      { name: 'Mirchi Masala', price: 49 },
      { name: 'Aloo Masala', price: 49 },
      { name: 'Aratikaya Masala', price: 49 },
      { name: 'Tomato Masala', price: 49 },
      { name: 'Egg Masala', price: 59 },
      { name: 'Batani Masala', price: 49 },
    ]
  },
  {
    id: 'specials',
    icon: '🍽️',
    title: 'Specials',
    subtitle: 'stall favourites',
    items: [
      { name: 'Batani Chat', price: 59 },
      { name: 'Egg Cutlet', price: 59 },
      { name: 'Cut Mirchi', price: 49 },
    ]
  }
];

export default function BajjiMixtureMenuModal({ isOpen, onClose }) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto bg-black/85 backdrop-blur-md">
        ...
        {/* Vintage Bhimavaram Muntha Masala Menu Card Container */}
        <motion.div
        ...
        >
          ...
          <div className="relative border-2 border-[#C08A34]/40 rounded-xl p-5 sm:p-8 bg-[#F7EFDB] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-amber-100/30 via-transparent to-transparent">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 mb-6 pb-4 border-b border-[#2A1810]/15">
              {BAJJI_MIXTURE_SECTIONS.map((sec) => (
                <button
                  key={sec.id}
            ...
            {/* ACTIVE SECTION PRICED ITEMS DISPLAY WITH DOTTED LEADERS */}
            <div className="max-h-[50vh] overflow-y-auto pr-1 custom-scrollbar">
              {BAJJI_MIXTURE_SECTIONS.filter(s => s.id === activeTab).map((sec) => (
                <div key={sec.id} className="space-y-4">
                  ...
                  <div className="space-y-1">
                    {sec.items.map((item, idx) => (
                      <div
                        key={idx}
                        className="flex items-baseline gap-2 py-2.5 border-b border-dashed border-[#2A1810]/15 last:border-none"
                      >
                        <span className="font-festive font-semibold text-base text-[#2A1810] shrink-0">
                          {item.name}
                        </span>
                        <span className="flex-1 border-b border-dotted border-[#8E6524]/40 mb-1"></span>
                        <span className="font-serif font-bold text-base text-[#B0402C] shrink-0">
                          ₹{item.price}
                        </span>
                      </div>
                    ))}
            ...
            <div className="mt-6 pt-4 border-t border-[#2A1810]/15 text-center">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#5A4632]/80 block mb-4">
                One Corner · Every Mix · Always Masala
              </span>
