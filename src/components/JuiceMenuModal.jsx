...
import { X, ArrowLeft, GlassWater, Sparkles, CheckCircle2 } from 'lucide-react';

export const JUICE_MENU_SECTIONS = [
  {
    id: 'juices',
    icon: '🥤',
    title: 'Fruit Juices',
    subtitle: 'fresh squeezed, no shortcuts',
    items: [
      'Watermelon Juice',
      'Mango Juice',
      'Orange Juice',
  ...
  },
  {
    ...
    icon: '🥛',
    title: 'Milkshakes',
    subtitle: 'thick, chilled & blended',
    items: [
      'Vanilla Milkshake',
      'Chocolate Milkshake',
...
export default function JuiceMenuModal({ isOpen, onClose }) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto bg-black/85 backdrop-blur-md">
        ...
        {/* Vintage Juices & Milkshakes Menu Card Container */}
        <motion.div
        ...
        >
          ...
          <div className="relative border-2 border-[#C08A34]/40 rounded-xl p-5 sm:p-8 bg-[#F7EFDB] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-amber-100/30 via-transparent to-transparent">
            ...
            <div className="text-center mb-6">
              ...
              {/* Animated Steam / Juice Bubble SVG Graphic */}
            ...
            {/* CATEGORY NAV TABS */}
            <div className="flex flex-wrap items-center justify-center gap-3 mb-6 pb-4 border-b border-[#2A1810]/15">
              {JUICE_MENU_SECTIONS.map((sec) => (
                <button
                  key={sec.id}
                  onClick={() => setActiveTab(sec.id)}
                  className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-200 flex items-center gap-2 ${
                    activeTab === sec.id
            ...
            <div className="max-h-[50vh] overflow-y-auto pr-1 custom-scrollbar">
              {JUICE_MENU_SECTIONS.filter(s => s.id === activeTab).map((sec) => (
            ...
            <div className="mt-6 pt-4 border-t border-[#2A1810]/15 text-center">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#5A4632]/80 block mb-4">
                One Corner · Every Sip · Always Refreshing
              </span>
