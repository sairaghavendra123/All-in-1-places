...
export const SHAWARMA_MENU_SECTIONS = [
  {
    id: 'rolls',
    icon: '🌯',
    title: 'Shawarma Rolls',
    subtitle: 'flame-roasted & wrapped fresh',
    items: [
      'Classic Chicken Shawarma Roll',
      'Special Rumali Chicken Shawarma',
      'Cheese Chicken Shawarma Roll',
      'Spicy Guntur Chilli Shawarma Roll',
      'Double Meat Loading Shawarma Roll'
    ]
  },
  {
    id: 'plates',
    icon: '🍽️',
    title: 'Shawarma Platters',
    subtitle: 'served with kubboos & garlic mayo',
    items: [
      'Chicken Shawarma Plate (with Kubboos & Dips)',
      'Open Meat Shawarma Platter',
      'Peri Peri Chicken Shawarma Plate',
      'Garlic Mayo Special Shawarma Plate'
    ]
  },
  {
    id: 'dips',
    icon: '🧄',
    title: 'Sides & Dips',
    subtitle: 'fresh garnishes & house sauces',
    items: [
      'Creamy Garlic Mayo Dip',
      'Fiery Guntur Red Chilli Sauce',
      'Pickled Veggies & Spiced Onions'
    ]
  }
];

export default function ShawarmaMenuModal({ isOpen, onClose }) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto bg-black/85 backdrop-blur-md">
        ...
        {/* Vintage Shawarma Menu Card Container */}
        <motion.div
        ...
        >
          ...
          <div className="relative border-2 border-[#C08A34]/40 rounded-xl p-5 sm:p-8 bg-[#F7EFDB] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-amber-100/30 via-transparent to-transparent">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 mb-6 pb-4 border-b border-[#2A1810]/15">
              {SHAWARMA_MENU_SECTIONS.map((sec) => (
                <button
                  key={sec.id}
            ...
            <div className="max-h-[50vh] overflow-y-auto pr-1 custom-scrollbar">
              {SHAWARMA_MENU_SECTIONS.filter(s => s.id === activeTab).map((sec) => (
            ...
            <div className="mt-6 pt-4 border-t border-[#2A1810]/15 text-center">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#5A4632]/80 block mb-4">
                One Corner · Every Roll · Always Flame Roasted
              </span>
